...
from algorithms.lattice import LatticePPOAgent
...
while episode_cnt < episode_num:
    ...
    while step_cnt < step_num:
        ...
        if not system.render_obj.pause:
            ...
            action_ris, logp_ris = agent_ris.act(observarion_ris)  # 0.1 is action noise factor
            action_uav, logp_uav = agent_uav.act(observarion_uav)
            if if_BS:
                action_uav[0] = 0
                action_uav[1] = 0
            ...
            agent_ris.store_transition(observarion_ris, action_ris, logp_ris, reward, next_state_ris, int(done))
            agent_uav.store_transition(observarion_uav, action_uav, logp_uav, reward, next_state_uav, int(done))
