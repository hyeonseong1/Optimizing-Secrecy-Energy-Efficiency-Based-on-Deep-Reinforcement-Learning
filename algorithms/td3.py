...
class CriticNetwork(nn.Module):
    def __init__(self, beta, input_dims, fc1_dims, fc2_dims, fc3_dims, fc4_dims, n_actions, name,
        ...
        torch.nn.init.uniform_(self.q.weight.data, -f5, f5)
        torch.nn.init.uniform_(self.q.bias.data, -f5, f5)
...
#        torch.nn.init.uniform_(self.mu.weight.data, -f5, f5)
#        torch.nn.init.uniform_(self.mu.bias.data, -f5, f5)
...
class TD3Agent(object):
    ...
    def act(self, observation, greedy=0.5, epsilon = 1):
        ...
        mu_prime = mu + torch.tensor(greedy * self.noise(),
    ...
    def learn(self):
        ...
        critic_value_ = torch.min(critic_value_1_, critic_value_2_)
        
        target = []
        for j in range(self.batch_size):
            target.append(reward[j] + self.gamma*critic_value_[j]*done[j])
        target = torch.tensor(target).to(self.critic_1.device)
