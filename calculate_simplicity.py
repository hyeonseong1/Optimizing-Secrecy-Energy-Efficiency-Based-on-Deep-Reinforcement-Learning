...
from metrics.compute_simpl.simpl_ddpg import compute_simpl_ddpg
from metrics.compute_simpl.simpl_simba import compute_simpl_simba
from metrics.compute_simpl.simpl_td3 import compute_simpl_td3
from metrics.compute_simpl.simpl_ppo import compute_simpl_ppo
...
def parse_ppo(text: str):
    actor = float(re.search(r"PPO\s*Actor\s*simplicity\s*:\s*([0-9.]+)", text, re.I).group(1))
    critic = float(re.search(r"PPO\s*Critic\s*simplicity\s*:\s*([0-9.]+)", text, re.I).group(1))
    return actor, critic
...
def run_n_times(n=10):
    ...
    for _ in range(n):
        ...
        _, t = capture_stdout(compute_simpl_ppo)
        a, c = parse_ppo(t)
        res["PPO"]["Actor"].append(a)
        res["PPO"]["Critic"].append(c)
    return res
...
def print_table(stats):
    headers = ["", "Simba", "TD3", "DDPG", "PPO"]
    actor_row  = ["Actor",  fmt(stats["Simba"]["Actor"]), fmt(stats["TD3"]["Actor"]), fmt(stats["DDPG"]["Actor"]), fmt(stats["PPO"]["Actor"])]
    critic_row = ["Critic", fmt(stats["Simba"]["Critic"]), fmt(stats["TD3"]["Critic"]), fmt(stats["DDPG"]["Critic"]), fmt(stats["PPO"]["Critic"])]
